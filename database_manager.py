...
class DatabaseManager():
    ...
    def save_to_page_embeddings(self, data):
        ...
        with self.conn.begin() as trans:
            try:
                query = text(f"SELECT * FROM parsed_pages_2 where page_number={data['page_number']} AND paper_id={data['paper_id']}")

                result = self.conn.execute(query).fetchall()

                if len(result) > 0:
                    
                    update_stmt = parsed_pages.update().where((parsed_pages.c.page_number == data['page_number']) & (parsed_pages.c.paper_id == data['paper_id'])).values(
                        paper_id=data['paper_id'],
                        url=data['url'],
                        page_number=data['page_number'],
                        paper_id_page_number=int(str(data['paper_id']) + str(data['page_number'])),
                        content=data['content'],
                        page_embeddings=data['embeddings']
                    )
                    
                    self.conn.execute(update_stmt)
                    print(f"Successfully updated page {data['page_number']} for paper ID {data['paper_id']}")
                    ...
                    print(f"Successfully inserted page {data['page_number']} for paper ID {data['paper_id']}")


                    
            # except IntegrityError:
            #     print(f"Page {data['page_number']} for paper ID {data['paper_id']} already exists. Skipping.")
            except Exception as e:
                print(f"Could not insert page {data['page_number']} for paper ID {data['paper_id']}. Error: {str(e)}")
